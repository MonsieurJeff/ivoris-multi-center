...
from ..core.introspector import get_schema, clear_cache
...
def cmd_discover(args, config):
    ...
    for center in config.centers:
        ...
        try:
            schema = get_schema(center.id, conn_str, use_cache=False)
            print(f"\n{'='*60}")
            print(f"Center: {center.name} ({center.id})")
            print(f"Database: {center.database}")
            print(f"Suffix: {schema.suffix}")
            print(f"{'='*60}")
            
            for table_name, table in schema.tables.items():
                print(f"\n  {table_name} -> {table.actual_name}")
                for canon, actual in table.columns.items():
                    if canon != actual:
...
def cmd_benchmark(args, config):
    ...
    for r in sorted(result.results, key=lambda x: x.duration_ms, reverse=True):
        ...
        print(f"  {status} {r.center_name:30} {r.duration_ms:6.0f}ms  ({len(r.entries)} entries)")
...
def main():
    ...
    extract_parser = subparsers.add_parser(
        "extract", help="Extract chart entries"
    )
    ...
    list_parser = subparsers.add_parser(
        "list", help="List configured centers"
    )
